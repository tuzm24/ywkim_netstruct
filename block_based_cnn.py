...
class Conv3x3_Recycle(nn.Module):
    def __init__(self,in_channels, out_channels, nth_layer):
        ...
        self.featuremap_size = block_size - 2*nth_layer
        self.upper_buffer = {}
...
def Copy_and_divide_Recycle(inputs, net, pad_required = 10, devide_size = 64):
    ...
    output = torch.zeros(b,c,h-pad_require, w-pad_require).double()
...
def Copy_and_divide_inputs(inputs, net, pad_required = 10, devide_size = 64):
    ...
    output = torch.zeros(inputs.shape).double()
    # output = np.full(inputs.shape, np.nan)
...
def basic_Copy_and_devide_input(inputs, net, pad_required = 1, devide_size = 64):
    ...
    output = torch.zeros(b, c, h-two_devide_size, w-two_devide_size).double()
    # output = np.full(inputs.shape, np.nan)
    img = F.pad(inputs, [pad_required, pad_required, pad_required, pad_required])
    for dx, dy, x, y in pos_list:
...
inputs = torch.randn(1,3,512, 512).double()
inputs = torch.autograd.Variable(inputs,requires_grad=False)

net = PlainNetwork(3, 3).double()
net.eval()

net_nopad = PlainNetwork_nopad(3,3).double()
net_nopad.eval()
# print(invest_net_pad(net,(3,100,100)))

net_recycle = PlainNetwork_nopad_recycle(3,3).double()
net_recycle.eval()

startTime = time.time()

c = Copy_and_divide_Recycle(copy.deepcopy(inputs), net_recycle)


print(time.time() - startTime)

startTime = time.time()
# b = net_nopad(F.pad(copy.deepcopy(inputs), [pad_require, pad_require, pad_require ,pad_require])).double()
...
startTime = time.time()
a = Copy_and_divide_inputs(copy.deepcopy(inputs), net_nopad)
print(time.time() - startTime)
