...
class TestNet(nn.Module):
    ...
    def __init__(self, num_input_features):
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(num_input_features, 64, kernel_size=3, stride=1,
...
class myDataBatch(Dataset, DataBatch):
    ...
    def unpackData(self, info):
        ...
        data = np.stack([*self.reshapeRecon(), modemap, depthmap], axis = 2)
...
if '__main__' == __name__:
    ...
    # optimizer = optim.SGD(net.parameters(), lr=dataset.cfg.INIT_LEARNING_RATE, momentum=0.9)
    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer=optimizer,
                                                  milestones=[int(dataset.cfg.OBJECT_EPOCH*0.5), int(dataset.cfg.OBJECT_EPOCH*0.75)],
                                                  gamma=0.1, last_epoch=-1)

    checkpoint = torch.load(NetManager.MODEL_PATH + '/model.pth')
