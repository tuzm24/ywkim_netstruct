...
class TestNet(nn.Module):
    ...
    def __init__(self, num_input_features):
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(num_input_features+4, 64, kernel_size=3, stride=1,
...
class myDataBatch(Dataset, DataBatch):
    ...
    def unpackData(self, info):
        ...
        zeroimg = [np.zeros((info[2], info[1]))]*4
        data = np.stack([*self.reshapeRecon(), *zeroimg], axis=2)
...
if '__main__' == __name__:
    ...
    # print(" ")
    train_loader = DataLoader(dataset=dataset, batch_size=dataset.batch_size, shuffle=False, num_workers=1)
    valid_loader = DataLoader(dataset=valid_dataset, batch_size=dataset.batch_size, shuffle=False, num_workers=1)
    ...
    net.to(device)
    summary(net, (dataset.data_channel_num+4, 132, 132))
    criterion = nn.L1Loss()
    MSE_loss = nn.MSELoss()
    ...
    for epoch_iter, epoch in enumerate(range(dataset.cfg.OBJECT_EPOCH), 1):
        ...
        for i, (recons, inputs, gts) in enumerate(train_loader, 0):
            ...
            for j in range(4):
                ...
                inputs[:, j + 3, :, :] += F.pad(torch.squeeze(outputs, 1),(2,2,2,2), mode='constant')
                outputs = net(inputs)
                loss = criterion(outputs, gts)
                MSE = MSE_loss(outputs, gts)
                optimizer.zero_grad()
            ...
            if i % dataset.PRINT_PERIOD == dataset.PRINT_PERIOD - 1:
                dataset.logger.info('[Training Epoch : %d, %5d/%d] loss: %.7f' %
                                    (epoch + 1, (i + 1) // dataset.PRINT_PERIOD * dataset.PRINT_PERIOD,
                                     len(train_loader), running_loss / dataset.PRINT_PERIOD))
                dataset.logger.info('[Training Epoch : %d, %5d/%d] Recon_loss: %.7f' %
                                    (epoch + 1, (i + 1) // dataset.PRINT_PERIOD * dataset.PRINT_PERIOD,
                                     len(train_loader), recon_loss / dataset.PRINT_PERIOD))
                running_loss = 0.0
                recon_loss = 0.0
