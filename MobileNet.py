...
class _DataBatch(DataBatch):
    ...
    def unpackData(self, info):
        ...
        data = np.stack([*self.reshapeRecon(), qpmap], axis=2)
...
if '__main__' == __name__:
    ...
    #                               num_workers=NetManager.NUM_WORKER, pin_memory=True)
    #     valid_loader = DataLoader(dataset=pt_valid_dataset, batch_size=dataset.batch_size, drop_last=True,
    #                               shuffle=False, num_workers=NetManager.NUM_WORKER, pin_memory=True)
    # else:
    train_loader = DataLoader(dataset=pt_dataset, batch_size=dataset.batch_size, drop_last=True, shuffle=True,
        ...
        for i in range(valid_dataset.batch_num):
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    if epoch_iter == 1:
                        tb.plotMap(dataset.ReverseNorm(inputs.split(1, dim=1)[3], idx=3).narrow(dim=2, start=2,
                                                                                                length=128).narrow(
                            dim=3, start=2, length=128), 'QP_Map', [22, 37], 4)
